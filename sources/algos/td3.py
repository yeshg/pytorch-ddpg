...
"""
TD3 replay buffer, model, utils
"""
from sources.replay_buffer import DDPGBuffer, Transition
from sources.model import Actor, Critic
from sources.utils import soft_update, hard_update


class TD3(object):
    def __init__(self, gamma, tau, batch_size=100, policy_noise=0.2, noise_clip=0.5, update_freq=2, hidden_size, num_inputs, action_space):
    ...
    def select_action(self, state, action_noise=None, param_noise=None):
        """
        Select action with target actor network and add clipped noise (target policy smoothing)
        """
